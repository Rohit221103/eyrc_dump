...
class Swift():
    ...
    def pid(self):
        ...
        d = [0, 0, 0]
        for indx in range(3):
            self.error[indx] = self.drone_position[indx] - self.setpoint[indx]
        ...
        self.cmd.rcRoll = 1500 - (p[0] + i[0] + d[0])
        self.cmd.rcPitch = 1500 + (p[1] + i[1] + d[1])
        self.cmd.rcThrottle = 1550 + (p[2] + i[2] + d[2])
...
def callback(data):
    ...
    cv.waitKey(1)
def cord_translate(shape,led_cx_cy_ar): # note that led_cx_cy_ar is a list containing 1 set of cords and area 
    a=shape                                  # a=(img_height,img_length)
    b=(int(a[1]/2),int(a[0]/2))
    tr_px=( led_cx_cy_ar[0] - b[0]  , b[1] - led_cx_cy_ar[1])
    scale=(a[1]/(8.6*2),a[0]/(8.6*2))  # 8.6  as drone_cam size/grid size is 8.6 by 8.6 #note that by 2 as 0,0 is new centre,so endpt is 4,4 not 8,8
...
if __name__ == '__main__':
    ...
    shape = ()
    swift_drone = Swift()
    r = rospy.Rate(25) # initaly it was 20 specify rate in Hz based upon your desired PID sampling time, i.e. if desired sample time is 33ms specify rate as 30Hz
    l=[[0,0,24.68],[8.3,0,24.68],
       [8.3,8.3,24.68],[0,8.3,24.68],[-8.3,8.3,24.68],
       [-8.3,0,24.68],
       [-8.3,-8.3,24.68],[0,-8.3,24.68],[8.3,-8.3,24.68],[8.3,-8.3,37],[11,11,37]]
    i=0
    rospy.sleep(5.)
    rospy.Subscriber("/swift/camera_rgb/image_raw",Image,callback)
    while not rospy.is_shutdown():
        if math.fabs(swift_drone.error[0])<=0.2 and  math.fabs(swift_drone.error[1])<=0.2 and  math.fabs(swift_drone.error[2])<=0.2 and i<len(l):
            swift_drone.setpoint=l[i]
            i+=1
        swift_drone.pid()
        r.sleep()
